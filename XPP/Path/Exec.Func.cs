...
{
  private Value FuncValue(int idx, XPNavigator ctx)
  {
  ...
  private Value LibraryFuncLast(scoped Span<Value> args, XPNavigator ctx) =>
    throw new NotImplementedException();
  private Value LibraryFuncPosition(scoped Span<Value> args, XPNavigator ctx) =>
    throw new NotImplementedException();

  private Value LibraryFuncCount(scoped Span<Value> args, XPNavigator ctx)
  {
    AssertArgType("count", 0, XPValueType.NodeSet, in args[0]);

    var idx = args[0].NodeSet;
    var count = 0;
    while (NextNode(idx, out _)) count++;
    return new(count);
  }

  private Value LibraryFuncId(scoped Span<Value> args, XPNavigator ctx) =>
    throw new NotImplementedException();

  private Value LibraryFuncLocalName(scoped Span<Value> args, XPNavigator ctx)
  {
    if (args.Length > 0)
    {
      AssertArgType("local-name", 0, XPValueType.NodeSet, in args[0]);
      NextNode(args[0].NodeSet, out ctx);
    }
    return new(ctx.Node.Name.Local);
  }

  private Value LibraryFuncNamespaceUri(scoped Span<Value> args, XPNavigator ctx)
  {
    if (args.Length > 0)
    {
    ...
    return new(ctx.Node.Name.NsUri);
  }

  private Value LibraryFuncName(scoped Span<Value> args, XPNavigator ctx)
  {
    if (args.Length > 0)
    {
    ...
    var name = ctx.Node.Name;
    if (name.Prefix.Length > 0)
      return new($"{name.Prefix}:{name.Local}");
    return new(name.Local);
  }

  private Value LibraryFuncString(scoped Span<Value> args, XPNavigator ctx)
  {
    if (args.Length > 0)
      return new(StringValue(args[0]));
    return new(NodeStringValue(ctx));
  }

  private Value LibraryFuncConcat(scoped Span<Value> args, XPNavigator ctx) =>
    throw new NotImplementedException();
  private Value LibraryFuncStartsWith(scoped Span<Value> args, XPNavigator ctx) =>
    new(StringValue(args[0]).StartsWith(StringValue(args[1])));
  private Value LibraryFuncContains(scoped Span<Value> args, XPNavigator ctx) =>
    new(StringValue(args[0]).Contains(StringValue(args[1])));

  private Value LibraryFuncSubstringBefore(scoped Span<Value> args, XPNavigator ctx)
  {
  ...
  private Value LibraryFuncSubstringAfter(scoped Span<Value> args, XPNavigator ctx)
  {
    var arg0 = StringValue(args[0]);
  ...
  private Value LibraryFuncSubstring(scoped Span<Value> args, XPNavigator ctx)
  {
  ...
  private Value LibraryFuncStringLength(scoped Span<Value> args, XPNavigator ctx)
  {
    if (args.Length > 0)
      return new(StringValue(args[0]).Length);
    return new(NodeStringValue(ctx).Length);
  }

  private Value LibraryFuncNormalizeSpace(scoped Span<Value> args, XPNavigator ctx)
  {
    var str = args.Length > 0 ? StringValue(args[0]) : NodeStringValue(ctx);
  ...
  private Value LibraryFuncTranslate(scoped Span<Value> args, XPNavigator ctx) =>
    throw new NotImplementedException();
  private Value LibraryFuncBoolean(scoped Span<Value> args, XPNavigator ctx) =>
    new(BoolValue(args[0]));
  private Value LibraryFuncNot(scoped Span<Value> args, XPNavigator ctx) =>
    new(!BoolValue(args[0]));
  private Value LibraryFuncTrue(scoped Span<Value> args, XPNavigator ctx) => new(true);
  private Value LibraryFuncFalse(scoped Span<Value> args, XPNavigator ctx) => new(false);
  private Value LibraryFuncLang(scoped Span<Value> args, XPNavigator ctx) =>
    throw new NotImplementedException();

  private Value LibraryFuncNumber(scoped Span<Value> args, XPNavigator ctx)
  {
    if (args.Length > 0)
      return new(NumberValue(args[0]));
    return new(NodeNumberValue(ctx));
  }

  private Value LibraryFuncSum(scoped Span<Value> args, XPNavigator ctx)
  {
    AssertArgType("sum", 0, XPValueType.NodeSet, in args[0]);
    var sum = 0.0;
    var idx = args[0].NodeSet;
    while (NextNode(idx, out var nav))
      sum += NodeNumberValue(nav);
    return new(sum);
  }

  private Value LibraryFuncFloor(scoped Span<Value> args, XPNavigator ctx) =>
    new(Math.Floor(NumberValue(args[0])));
  private Value LibraryFuncCeiling(scoped Span<Value> args, XPNavigator ctx) =>
    new(Math.Ceiling(NumberValue(args[0])));
  private Value LibraryFuncRound(scoped Span<Value> args, XPNavigator ctx) =>
