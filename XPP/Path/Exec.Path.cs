...
{
  public static readonly ExecCtxSet One = new() { Length = 1 };

  public int Length = 0;
}

public readonly struct ExecPathCtx(XPNavigator Nav, ExecCtxSet Set, int Position)
{
  public readonly XPNavigator Nav = Nav;
  public readonly ExecCtxSet Set = Set;
  public readonly int Positon = Position;
}
  ...
  public override bool Next(out ExecPathCtx next)
  {
    while (true)
    {
      ...
      if (!Map(pctx, out var nextNav))
        continue;
      next = new(nextNav, ExecCtxSet.One, 0);
      return true;
    }
  }
...
public abstract class ExecPathOpSingle() : ExecPathOp(null, true)
{
  ...
  public override bool Next(out ExecPathCtx next)
  {
    ...
    next = new(SingleNav, ExecCtxSet.One, 0);
    return !(first = false);
  }
...
public class ExecPathOpDedupe(ExecPathOp Parent) : ExecPathOp1To1(Parent)
{
  ...
  {
    if (!last.SameAs(last = from.Nav.Node))
    {
      to = from.Nav;
      return true;
    }
...
public class ExecPathOpUnion(ExecPathOp Left, ExecPathOp Right) : ExecPathOp(null, true)
{
  ...
  public override bool Next(out ExecPathCtx next)
  {
    ...
    var cmp = leftNext.Nav.CompareTo(rightNext.Nav);
...
public class ExecPathOpExpr(ExecExprOp Expr) : ExecPathOp(null, true)
{
  ...
  public override bool Next(out ExecPathCtx next)
  {
    if (first)
    {
      var val = Expr.Value(new(navCtx, ExecCtxSet.One, 0));
  ...
  : ExecPathOp1To1(Parent)
{
  ...
  protected override bool Map(ExecPathCtx from, out XPNavigator to)
  {
    ...
    if (val.Type switch
    {
      XPValueType.Number => from.Positon + 1 == val.Number,
      _ => val.BoolValue,
    })
    {
      to = from.Nav;
